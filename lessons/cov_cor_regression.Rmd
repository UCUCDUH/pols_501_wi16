...
3. The regression output `reg` contains more information than the one that the `summary()` function returns. To take a look at the differen info in `reg`, use the `str()` function. You can do the same with the output of a large number of statistical tests in `R`.

```{r}
str(reg)
```

You can pull out any of these elements within the regression output object using the `$` dolar sign. Or you can also use the dolar sign to pull out an object from the `summary` of `reg` (`summary(reg)`).
