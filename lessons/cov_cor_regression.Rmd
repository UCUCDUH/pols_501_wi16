...
In this lab we'll look at different methods to study the relationship between variables. 
...
One of the analytical downsides of calculating the covariance to study the relationship between two variables is that the resulting estimator is scale-dependent and it's hard to compare it to other covariance estimators. To address this issue, we often calculate instead the `correlation` of two variables; which is a standardized statistic that ranges from -1 to 1. Positive correlation values indicate a positive relationship and negative values a negative one. The closer the statistic is to -1 or to 1, the stronger the relationship between these two variables. 
...
prestige_cor_new$var2 <- rep(colnames(prestige_cor), ncol(prestige_cor))
...
3. The regression output `reg` contains more information than the one that the `summary()` function returns. To take a look at the different info in `reg`, use the `str()` function. You can do the same with the output of a large number of statistical tests in `R`.

```{r}
str(reg)
```

[EDIT] Explain the parts of the str


You can pull out any of these elements within the regression output object using the `$` dollar sign. Or you can also use the dollar sign to pull out an object from the `summary` of `reg` (`summary(reg)`).
...
The package `broom` has some functions to pull out and transform to friendly `data.frames` the output of statistical tests (e.g. `t.test`, `lm`, etc.). `broom` has 3 main functions:
  ...
  1. `tidy`: Transforms into a ready-to-go `data.frame` the coefficients, standard errors (and confidence intervals if given), critical values, and p-values in statistical tests' outputs
...
EDIT: explaing the columns of these data frames.

For example, we can use the new dataset we created using the `augment()` function (`new_Prestige`) to plot the values of `income` predicted by the model (with 95% confidence intervals) v. the actual values of `income`.

```{r, fig.keep='all'}
ggplot(new_Prestige, aes(x = income, y = .fitted)) + 
  geom_point() +
  geom_line(aes(x = income, y = income), linetype = "dashed") +
