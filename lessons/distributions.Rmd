...
for the distributions are included in base R, several packges are used for
...
For example, the probabiilty that a standard normal random variable $X$ is greater than -1.96 is,
```{r}
...
```
This is eqivalent to 1 minus the CDF,
```{r}
...
known as a pseudo-randome number generator. In short, these are algorithms that 
are deterministic, but procude a sequence of numbers that have properties similar to 
