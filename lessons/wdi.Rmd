...
We first need to download the WDI data. This could be done manually, but I 
will do it using R commands for maximum replicability.
...
We use the `matches` function to only match the variables beginning with "X", which are the year variables,
```{r}
wdi_data_long <-
  wdi_data %>%
...
We will remove the "X" using the function `str_replace` from the **stringr** package,
and convert it to an integer vector.
```{r}
